...
def read_dock_fp_out(file, max_num):
    ...
    for line in file1:
         linesplit = line.split(':') #split on colon
         if (len(linesplit) > 1):
             if ("Name" in linesplit[0]):
                 temp_name_dock = str(linesplit[1].strip())
                 command = "grep -m 1 " + temp_name_dock + " " + " | awk -F, '{print $2}'"
                 p = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE)
                 temp_rot_bond, error = p.communicate()
                 temp_rot_bond.strip()
             if ("desc_FPS_vdw_energy" in linesplit[0]):
                 temp_vdw = float(linesplit[1].strip())
             if ("desc_FPS_es_energy" in linesplit[0]):
                 temp_es = float(linesplit[1].strip())
             if ("desc_FPS_vdw+es_energy" in linesplit[0]):
                 temp_totalScore = float(linesplit[1].strip()) 
             if ("desc_FPS_num_hbond" in linesplit[0]):
                 temp_hbond = int(linesplit[1].strip())
             if ("desc_FPS_vdw_fps" in linesplit[0]):
                 temp_vdw_fp = float(linesplit[1])
             if ("desc_FPS_es_fps" in linesplit[0]):
                 temp_es_fp = float(linesplit[1])
             if ("desc_FPS_hb_fps" in linesplit[0]):
                 temp_hb_fp = float(linesplit[1])
             if ("desc_FPS_vdw_fp_numres" in linesplit[0]):
                 temp_vdw_fp_numres = float(linesplit[1])
             if ("desc_FPS_es_fp_numres" in linesplit[0]):
                 temp_es_fp_numres = float(linesplit[1])
             if ("desc_FPS_hb_fp_numres" in linesplit[0]):
                 temp_hb_fp_numres = float(linesplit[1])
             if ("Pharmacophore_Score" in linesplit[0]):
	         temp_ph4 = float(linesplit[1])
             if ("Property_Volume_Score" in linesplit[0]):
                 temp_vo = float(linesplit[1])
             if ("Tanimoto_Score" in linesplit[0]):
                 temp_tan = float(linesplit[1])
             if ("Hungarian_Matching_Similarity_Score" in linesplit[0]):
                 temp_hun = float(linesplit[1])
             if ("Descriptor_Score" in linesplit[0]):
                 temp_dscrpt = float(linesplit[1])
             if ("Internal_energy_repulsive" in linesplit[0]):
                 temp_internal_energy = float(linesplit[1])
...
def main():
    ...
    outputfilename_list  = outputfileprefix + "_sorted_totScore_" + str(max_num) + ".csv";
