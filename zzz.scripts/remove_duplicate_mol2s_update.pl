...
#Actually it chooses the molecule with best footprint score although the header says differently
#Needs further investigation because in the past the DCE score was not apart of the header mol2
#but now with the invention of the new Descriptor Score it is possible to have DCE score in
#the header file.
...
while (<INPUT>) {
	...
	if ($_ =~ m/Footprint_Similarity_Score:/){
		chomp ($_);
