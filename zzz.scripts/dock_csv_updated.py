...
class LIG_DATA:
    def __init__(self, name_dock, vdw,es, hbond, int_energy, vdw_fp, es_fp, hb_fp, totalScore, vdw_fp_numres, es_fp_numres, hb_fp_numres, rot_bond):
...
def write_LIG_DATA(data, file):
    file.write( str(data.name_dock) + "," + str(data.vdw) + "," + str(data.es) + "," + str(data.int_energy) + "," + str(data.hbond) + "," + 
                ...
                str(data.es_fp_numres) + "," + str(data.hb_fp_numres) + "," + str(data.rot_bond))
         ...
         if (len(linesplit) > 2):
             ...
             if (linesplit[1] == "hb_fp_numres:"):
                 ...
                 data = LIG_DATA(temp_name_dock, temp_vdw, temp_es, temp_hbond, temp_internal_energy, temp_vdw_fp, temp_es_fp,
                                 temp_hb_fp, temp_totalScore, temp_vdw_fp_numres, temp_es_fp_numres, temp_hb_fp_numres, temp_rot_bond)
                 list.append(data)
...
def write_csv(data, filename):
    ...
    filehandle.write("name_dock,vdw,es,int_energy,hbond,vdw_fp,es_fp,hb_fp,totalScore,vdw_fp_numres,es_fp_numres,hb_fp_numres,rot_bond\n")
