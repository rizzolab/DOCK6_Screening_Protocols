...
rm -rf ${rootdir}/${system}/008.rank_mol/${vendor}/totalScore_rank
mkdir -p ${rootdir}/${system}/008.rank_mol/${vendor}/totalScore_rank
cd ${rootdir}/${system}/008.rank_mol/${vendor}/totalScore_rank
...
##################################################
cat <<EOF >${system}.${vendor}.totalScore_rank_score.in
...
ligand_outfile_prefix                                        ${vendor}.totalScore.output
write_footprints                                             no
write_hbonds                                                 no
...
cat <<EOF >${system}.${vendor}.totalScore_rank.qsub.csh
#!/bin/tcsh
#PBS -l walltime=${wcl}
#PBS -l nodes=${nodes}:ppn=${ppn}
#PBS -N ${system}.vs.totalScore_rank
#PBS -q ${queue}
#PBS -V
#PBS -j oe
...
${dockdir}/dock6.mpi -v \
-i ${system}.${vendor}.totalScore_rank_score.in \
-o ${system}.${vendor}.totalScore_rank_score.out
...
#SBATCH --nodes=${nodes}
#SBATCH --ntasks=24
#SBATCH --job-name=${system}.vs.totalScore_rank
#SBATCH --output=${system}.vs.totalScore_rank
#SBATCH -p rn-long
...
### Submit the job
echo "Submitting ${system}.${vendor}.totalScore_rank.qsub.csh"
qsub ${system}.${vendor}.totalScore_rank.qsub.csh > & ${system}.${vendor}.totalScore_rank.qsub.log
date
